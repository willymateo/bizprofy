...
const Table = ({ className = "", warehouse }: Props) => {
  const limit = useSelector(
    (state: Store) => state?.stockIn?.filters?.[warehouse?.id ?? ""]?.limit ?? PAGE_SIZE_OPTIONS[0],
  );
  ...
  const transactionDateLessThanOrEqualTo = useSelector(
    (state: Store) =>
      state?.stockIn?.filters?.[warehouse?.id ?? ""]?.transactionDateLessThanOrEqualTo ||
      dayjs().endOf("day").toISOString(),
  );
  const offset = useSelector(
    (state: Store) => state?.stockIn?.filters?.[warehouse?.id ?? ""]?.offset ?? 0,
  );
  ...
  const [data, setData] = useState<GetStockInResponse>({
    summarizedData: {
      totalQuantity: 0,
      totalCostSum: 0,
    },
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const stockData = await getStockIn({
