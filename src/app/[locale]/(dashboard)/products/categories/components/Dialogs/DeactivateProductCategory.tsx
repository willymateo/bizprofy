...
const DeactivateProductCategory = ({
  ...
  return (
    <Dialog onClose={onClose} open={isOpen}>
      <DialogTitle>{t("Deactivate product category")}</DialogTitle>

      <DialogContent className="flex flex-col gap-5 items-center">
        {error ? (
          <Alert variant="filled" severity="error" className="w-full">
            {t("An error occurred while deactivating the product category Please try again later")}.
          </Alert>
        ) : null}

        <DialogContentText>
          {`${t("Are you sure you want to deactivate the product category")} `}
          <strong>{name}</strong>?
        </DialogContentText>
