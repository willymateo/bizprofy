...
const ActivateProductCategory = ({
  ...
  return (
    <Dialog onClose={onClose} open={isOpen}>
      <DialogTitle>{t("Activate product category")}</DialogTitle>

      <DialogContent className="flex flex-col gap-5 items-center">
        {error ? (
          <Alert variant="filled" severity="error" className="w-full">
            {t("An error occurred while activating the product category Please try again later")}.
          </Alert>
        ) : null}

        <DialogContentText>
          {`${t("Are you sure you want to activate the product category")} `}
          <strong>{name}</strong>?
        </DialogContentText>
